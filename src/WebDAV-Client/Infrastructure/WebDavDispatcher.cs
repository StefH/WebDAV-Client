...
internal class WebDavDispatcher : IWebDavDispatcher, IDisposable
{
    ...
    public async Task<HttpResponse> SendAsync(Uri requestUri, HttpMethod method, RequestParameters requestParams, CancellationToken cancellationToken)
    {
        ...
        return new HttpResponse(response.Content, (int)response.StatusCode, response.ReasonPhrase ?? string.Empty);
    }
